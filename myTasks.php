			...
			<div class="_container">
				...
				<section class="myTasks">
					<?php
						...
						$stmt = $mysqli->prepare("SELECT tasks.id, tasks.title, tasks.description, tasks.due_date, tasks.date_from, tasks.date_perform, tasks.date_end, tasks.status, tasks.creator_id, tasks.assignee_id, tasks.comment, users.username AS assigner_username, CONCAT(users.name, ' ', users.surname, ' ', users.third_name) AS full_name FROM tasks INNER JOIN users ON tasks.creator_id = users.id WHERE tasks.assignee_id = ? ORDER BY tasks.id DESC");
						...
						// Отображение задач
						print "<h2 style='margin-bottom: 20px;'>Ваши задачи:</h2>
						...
						if ($result->num_rows > 0) {
								...
								print '
									<div class="task-card '.$class_status.'">
										...
										<div class="task_right_side">
											...
											} else if ($r_date_end != '...' && $_SESSION['username'] == $creator_username && $creator_username != $row['assigner_username']) {
												print '
												<form action="rate.php" method=post>
													<input type=hidden name=page value="myTasks.php">
													<input type=hidden name=task_id value=' . $row['id'] . '>
													<input type=hidden name=username value='.$row['assigner_username'].'>
							...
							print "У вас пока нет задач! Отдыхайте :)";
						}
